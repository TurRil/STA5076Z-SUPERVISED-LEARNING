...
ll <- as.data.frame(cbind(c('lm', 'knn', 'brnn', 'ridge', 'rlm', 'glm', 'lasso', 'rvmPoly', 'rf', 'glmboost','svmPoly'), 0))
names(ll) <- c('name','time')


#fc <- file('results/student_results.csv')
for (i in 1:nrow(ll)) {
  
  mod.file <- paste("results/student_", ll[i, 1], ".rds", sep="")
  ...
  if (file.exists(mod.file)) {
    ...
    #writeLines(paste(accuracy, runTime, sep = ";"), fc)
  }
...
test_pred <- predict(student.knn, newdata = testing)

confusionMatrix(test_pred, testing$G3)
```